...
import com.luyouchina.train.core.common.util.Constants;
 ...
 */
@ServiceModule("bas")
public class BasFindPageListService {
	private BasicServiceConfRepository basicServiceConfRepository;
	public BasFindPageListService() {
		 basicServiceConfRepository = SingleMongoFactory.getBean(BasicServiceConfRepository.class, "service_findpagelist_conf");
	}

	// --------------------------------------------------------------------------------------------------------
	/**
	 * 【XXXX】动态通用查询    传入json 格式如后面描述
	 ...
	 * 
	 */
	@ServiceMethod("getFindPageList")
	public ResponseObject getFindPageList(RequestObject request) {
		...
		int objectint = basicRepository.findCount(queryObj);
		if ("1".equals(param.get("page").toString())) {
			list.add(new BasicDBObject("_id", new BasicDBObject(QueryOperators.GT, new ObjectId(Constants._id0))));
		} else if (param.get("lastitemid") != null) {
			list.add(new BasicDBObject("_id", new BasicDBObject(QueryOperators.LT, new ObjectId(param.get("lastitemid").toString()))));
		}
		queryObj.put(QueryOperators.AND, list);
		List<DBObject> objectlist = basicRepository.findList(queryObj, sortObj, param.get("lastitemid"), (Integer) param.get("page"), (Integer) param.get("rows"));
		
		//7.addkeys    格式为 {aaa:bbb;ccc:ppp}
		//  调整为支持分层键的加入哈
		
		for (String Dbaddkey:((DBObject)object.get("addkeys")).keySet()) {							
			basicRepository.addKey(objectlist,"",Dbaddkey, ((DBObject)object.get("addkeys")).get(Dbaddkey).toString());
		}
		...
		for (DBObject embedkey:(List<DBObject>)object.get("embedkeys")) {
			...
			basicRepository.addCodeName(objectlist,"", embedkey.get("collection").toString(),embedkey.get("method").toString(),
		...
		for (String Dbreplacekey:((DBObject)object.get("replacekeys")).keySet()) {				
			basicRepository.replaceKey(objectlist,"",Dbreplacekey, ((DBObject)object.get("replacekeys")).get(Dbreplacekey).toString());
		}
		...
		for (String Dbnamerediskey:((DBObject)object.get("namerediskeys")).keySet()) {				
			basicRepository.addCodeNameRedis(objectlist,"",Dbnamerediskey,((DBObject)object.get("namerediskeys")).get(Dbnamerediskey).toString());// 旧键、新键
		}
		...
		basicRepository.removeFields(objectlist,"", removekeys);
	    ...
	    for (String Dbtransformkey:((DBObject)object.get("transformkeys")).keySet()) {
			...
			case "tofront":
				basicRepository.covertToFront(objectlist,"", Dbtransformkey);
				break;
			case "toback":
				basicRepository.covertToBack(objectlist,"", Dbtransformkey);
				break;
			case "toboolean":
				basicRepository.covertToBoolean(objectlist,"",Dbtransformkey);
				break;
			default:
				break;
			}
	    ...
	    objectlist=(List<DBObject>)basicRepository.returnFields(objectlist,null,"",returnkeys);
		//-----------------------------------------------------------------------------------
		Map<String, Object> remap = new HashMap<String, Object>();
		int rows = Integer.valueOf(param.get("rows").toString());
		remap.put("totalrows", objectint);
		remap.put("totalpage",(objectint + rows -1)/rows);// 总分页yuxh on 2015-9-6 调整分页算法
		remap.put("page", param.get("page"));
		remap.put("rows", param.get("rows"));
		remap.put("list", objectlist);
